...
| `use_layerwise` | Enable layer-by-layer KV save/load. Supported by the `memcache` and `mooncake` backends. MemCache is required for layerwise KV buffer reuse. The default value is false. |
